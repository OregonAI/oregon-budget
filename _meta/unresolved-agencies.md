...
**126 appropriations** across **26 distinct names** overlap the FY2019–FY2025 expenditure mirror but name an agency that does not resolve against the sibling registry's `budget_agency_code`, so no join was built.
...
**26 appropriations.** A registry entry with a budget code looks like the same body, usually differing only in word order ("State Forestry Department" vs "Department of Forestry"). **Suggestions are unverified** and were produced by exactly the fuzzy matching `resolve_agency` refuses to apply. Confirm one by recording it in the sibling's registry, not by loosening the matcher.

| bill says | appropriations | suggested registry entry | code | overlap |
|---|---:|---|---:|---:|
| State Forestry Department | 17 | `department-of-forestry` | 629 | 1.00 |
| Department of Land Conservation and Development | 5 | `land-conservation-and-development-department` | 660 | 1.00 |
| State Fire Marshal | 1 | `department-of-state-police-office-of-state-fire-marshal` | 260 | 0.67 |
| Early Learning Division of the Department of Education | 1 | `oregon-department-of-education-early-learning-division` | 588 | 1.00 |
| Oregon Department of Emergency Management | 1 | `oregon-military-department-office-of-emergency-management` | 258 | 0.67 |
| Department of the State Fire Marshal | 1 | `department-of-state-police-office-of-state-fire-marshal` | 260 | 0.67 |

## 3. No registry counterpart — correctly unresolved

**41 appropriations.** These bodies issue no administrative rules, so they hold no OAR chapter and do not appear in a registry keyed on chapter assignment. The Emergency Board is a contingency fund that disburses through other agencies; the Governor's office and the legislative-branch bodies are outside the executive rulemaking scheme entirely. Absence here is a fact about the registry's scope, not a gap to fill.

| bill says | appropriations | closest registry entry (no code / low overlap) |
|---|---:|---|
| Public Defense Services Commission | 7 | — |
| Emergency Board | 6 | — |
| Office of the Governor | 6 | — |
| Commission on Judicial Fitness and Disability | 4 | — |
| Oregon Advocacy Commissions Office | 4 | — |
| Youth Development Division | 2 | `oregon-department-of-education-youth-development-division` (0.67) |
...
| Oregon Patient Safety Commission | 1 | `oregon-patient-safety-commission` (1.00) |
| Department of Early Learning and Care | 1 | `oregon-department-of-education-early-learning-division` (0.50) |
| Elliott State Research Forest Authority | 1 | — |
| Invasive Species Council | 1 | `department-of-agriculture-oregon-invasive-species-council` (0.75) |
