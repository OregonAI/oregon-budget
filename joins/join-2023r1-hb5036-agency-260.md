...
- Agency identity: `department-of-state-police-office-of-state-fire-marshal` in the `executive-regulatory-frameworks` corpus, whose registry carries the hand-reviewed `das_agency_number: 260`. Resolved here by matching this bill's `appropriated_to` string against that registry, exact-only.
