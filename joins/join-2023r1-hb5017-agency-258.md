...
- Agency identity: `oregon-military-department-office-of-emergency-management` in the `executive-regulatory-frameworks` corpus, whose registry carries the hand-reviewed `das_agency_number: 258`. Resolved here by matching this bill's `appropriated_to` string against that registry, exact-only.
